...
namespace NoSqlDb
{
	...
	*/
	inline void showHeader(bool showKey = true, std::ostream& out = std::cout)
	{
		...
		out << std::setw(30) << std::left << "   Payload";
	...
	{
		out << "\n  ";
		out << std::setw(15) << std::left << el.name().substr(0, 100);
		out << std::setw(20) << std::left << std::string(el.dateTime());
		out << std::setw(20) << std::left << el.descrip().substr(0, 20);
		out << std::setw(20) << std::left << std::string(el.payLoad()).substr(0, 100);
	...
	{
		out << "\n  ";
		out << std::setw(30) << std::left << key.substr(0, 100) << "\t";
		out << std::setw(30) << std::left << el.name().substr(0, 100) << "    ";
		out << std::setw(30) << std::left << std::string(el.dateTime()) << "   ";
		out << std::setw(30) << std::left << el.descrip().substr(0, 30) << "  ";
		out << std::setw(30) << std::left << std::string(el.payLoad()).substr(0, 30) << "\t";
		Status status = el.getStatus();
		if (status == Open)
			out << std::setw(20) << std::left << "Open";
		else
			out << std::setw(20) << std::left << "Close";
		Children children = el.children();
		if (children.size() > 0)
		{
			out << "\n    child keys: ";
		...
		P payload = el.payLoad();
		//std::vector<std::string>category = payload.categories();
