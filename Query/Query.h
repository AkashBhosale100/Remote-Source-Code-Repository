...
namespace NoSqlDb
{
	...
	public:
		...
		int allCheck_InsWithinMonthInterval(DateTime from, DateTime to);
	...
	*/
	...
	template<typename P>
	int Query<P>::allCheck_InsWithinMonthInterval(DateTime from, DateTime to)
	{
		...
		if (fromMonth > toMonth)
		 ...
		 fromMonth = 7;
		 toMonth = 9;
		std::vector<int>months(12, 0);
		
		for (auto item : db_)
		{
			int month = item.second.dateTime().month();
			months[month]++;
		}
